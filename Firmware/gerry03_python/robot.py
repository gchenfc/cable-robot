...
class Robot:
    ...
    def IK(locs, pos):
        return tuple(Robot.dist(loc, pos) for loc in locs)
    def FK(locs, lengths):
        a = locs[0][0]-locs[3][0]
        b = abs(lengths[3])
        c = abs(lengths[0])
        ...
        err = tuple(abs((l - lhat) / l) for l,lhat in zip(lengths, Robot.IK(locs, pos)))
        return pos, err
    def getpos(self):
        if not self.is_calibrated():
            return (float('nan'),)*2, (float('nan'),)*4
        return Robot.FK([axis.location for axis in self.axes],
                        [axis.pos - axis.zero for axis in self.axes])
    def gotopos(self, pos, dry_run=False):
        ...
        if pos[0] < 0 or pos[0] > self.width or pos[1] < 0 or pos[1] > self.height:
            ...
            return
        ls = Robot.IK([axis.location for axis in self.axes], pos)
        fudge = 1.5
        ls = [-l + axis.zero + fudge for l, axis in zip(ls, self.axes)]
        ...
        while Robot.dist(tmppos, self.setpoint_pos) > 3:
            ...
            # IK
            ls = Robot.IK([axis.location for axis in self.axes], tmppos)
            fudge = 1.5
            ls = [-l + axis.zero + fudge for l, axis in zip(ls, self.axes)]
            self.trajectory.append(ls)
        ls = Robot.IK([axis.location for axis in self.axes], self.setpoint_pos)
        fudge = 1.5
        ls = [-l + axis.zero + fudge for l, axis in zip(ls, self.axes)]
        self.trajectory.append(ls)
    ...
    def movetowards(self):
        if self.trajectoryi >= len(self.trajectory):
            ...
            if (max(abs(l - ax.pos_corr()) for l,ax in zip(ls, self.axes)) < 9):
                self.trajectoryi += 1
    ...
    def import_calibration(self, fname='calib/20201201_0820.json'):
