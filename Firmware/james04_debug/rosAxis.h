...
private:
	...
	ros::Subscriber<std_msgs::Float32, RosAxis> sub_vel;
...
public:
  ...
  float last_curLim = 0;
	...
	RosAxis(uint16_t nodeID) :
		...
		sub_vel(assembleName("odrv/axis", nodeID, "/set_vel"), &RosAxis::sub_vel_cb, this),
	...
	void init() {
    // Serial1.print("initiating node id: ");
    // Serial1.println(nodeID_);
		nh.subscribe(this->sub_estop);
		nh.subscribe(this->sub_ctrl);
		nh.subscribe(this->sub_pos);
		nh.subscribe(this->sub_vel);
	...
	void sub_vel_cb(const std_msgs::Float32& vel){
		set_vel(vel.data, 0);
	};
