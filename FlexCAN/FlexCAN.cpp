...
int FlexCAN::write(const CAN_message_t &msg)
{
  ...
  for (int index = NUM_MAILBOXES - numTxMailboxes - 1; index < NUM_MAILBOXES; index++) {
    if ((FLEXCANb_MBn_CS(flexcanBase, index) & FLEXCAN_MB_CS_CODE_MASK) == FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_RX_EMPTY)) {
      // tx got converted, converting back (should be caught in interrupt but just in case)
      FLEXCANb_MBn_CS(flexcanBase, index) = FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_TX_INACTIVE);
      buffer = index;
      break;
    }
    if ((FLEXCANb_MBn_CS(flexcanBase, index) & FLEXCAN_MB_CS_CODE_MASK) == FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_RX_FULL)) {
      // tx got converted and is now full, this should never really happen bc ISR should empty the read buffer
      // desired behavior is not well defined so we'll just drop the data and prepare to transmit
      FLEXCANb_MBn_CS(flexcanBase, index) = FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_TX_INACTIVE);
      buffer = index;
      break;
    }
...
void FlexCAN::message_isr(void) 
{
    ...
    {        
        ...
        case 0xE: //remote request junk again. Go away.
            break;
        */  
        case 4: //rx empty already. this means a tx MB got converted to an rx MB at completion. convert back.
        ...
        case 6: //rx overrun. We didn't get there in time and a second frame tried to enter the MB. Whoops... Can probably still grab the frame though.
            ...
            if (i >= NUM_MAILBOXES - numTxMailboxes - 1) { // probably an RTR response where the rx MB didn't get converted back in time
              FLEXCANb_MBn_CS(flexcanBase, i) = FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_TX_INACTIVE);
            }
            break;
