...
std::vector<double> LQRFactorGraph::line_space(int start, int end, int n) {
...
std::vector<std::vector<double>> LQRFactorGraph::path_builder(
...
void LQRFactorGraph::add_level_to_intermediateForces(
...
gtsam::Vector LQRFactorGraph::vectorBuilder(int n, int l) {
  gtsam::Vector out(l);
  for (int i = 0; i < l; i++) {
    out(i) = n;
  }
...
void LQRFactorGraph::run() {
  ...
  for (int ti = 0; ti < timeline.size(); ti++) {
    ...
    gtsam::Matrix eyes[9] = {
        gtsam::Matrix(3, 3), gtsam::Matrix(3, 3), gtsam::Matrix(3, 3),
        gtsam::Matrix(3, 3), gtsam::Matrix(3, 3), gtsam::Matrix(3, 3),
        gtsam::Matrix(3, 3), gtsam::Matrix(3, 3), gtsam::Matrix(3, 3)};
    ...
    graph.push_back(boost::make_shared<gtsam::LinearContainerFactor>(
        gtsam::JacobianFactor(forces[ti * 4 + 0], eyes[0], forces[ti * 4 + 1],
                              eyes[1], intermediateForces[ti][0], -eyes[2],
                              vectorBuilder(0, 3),
                              gtsam::noiseModel::Isotropic::Variance(3, 0.1)),
        linPoint));
    graph.push_back(boost::make_shared<gtsam::LinearContainerFactor>(
