...
namespace cable_robot{
	class PositioningFactorGraph{
		private:
			std::vector<gtsam::Key> torque, tension, current, point, forces, forcesAnchor, pointEE, velEE, forceEE;
			...
			void run() {
				...
				for (int ti = 0; ti < timeline.size(); ti ++ ){
					for (int ci = 0; ci < 4; ci ++) {
						...
						//MotorFactor -> evalerror: actual torque and expected torque based on input current
						graph.push_back(boost::make_shared<manipulator::MotorFactor>(torque[ind],current[ind]
			            	, gtsam::noiseModel::Isotropic::Variance(1, 0.1)));
						//WrenchFactor -> torque*radius_of_motor - tension 
						graph.push_back(boost::make_shared<manipulator::WrenchFactor>(tension[ind],torque[ind]
					...
					// gtsam::Matrix eyes[9] = {gtsam::Matrix(3,3),gtsam::Matrix(3,3),gtsam::Matrix(3,3),gtsam::Matrix(3,3),gtsam::Matrix(3,3),gtsam::Matrix(3,3),gtsam::Matrix(3,3),gtsam::Matrix(3,3),gtsam::Matrix(3,3)};
					 gtsam::Matrix eyes[9] = {gtsam::I_3x3,gtsam::I_3x3,gtsam::I_3x3,gtsam::I_3x3,gtsam::I_3x3,gtsam::I_3x3,gtsam::I_3x3,gtsam::I_3x3,gtsam::I_3x3};
					...
					linPoint.insert(intermediateForces[ti][0], vectorBuilder(0,3)); // linear point. not so important
				    linPoint.insert(intermediateForces[ti][1], vectorBuilder(0,3));
				    linPoint.insert(forceEE[ti], vectorBuilder(0,3));
				    
				    //linear container factor -> wrap linear in a non linear. linear behavior black dot
				    graph.push_back(boost::make_shared<gtsam::LinearContainerFactor>(gtsam::JacobianFactor(
				        forces[ti*4 + 0], eyes[0], //identity matrix
